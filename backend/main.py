...
# OI computed ticks stay at 1-minute bars. Raw fetch runs every 15s so volume
# logs have 15-second granularity (the actual cadence is driven by the config
# key `fetch_interval_seconds`; this constant is for logging/reference only).
COMPUTE_TICK_INTERVAL = 60  # seconds between computed (OI) ticks — 1-minute bars
RAW_FETCH_INTERVAL = 15     # seconds between raw fetches — 15s volume granularity
