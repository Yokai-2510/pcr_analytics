...
@app.get("/api/total-volume/{instrument}")
def total_volume(
    instrument: str,
    date: str = Query(default_factory=utils.today_ist),
) -> list[dict[str, Any]]:
...
def option_chain(
    ...
    return data_processor.get_option_chain_latest(_instrument_or_404(instrument), date)


@app.get("/api/sr-history/{instrument}")
def sr_history(
    instrument: str,
    date: str = Query(default_factory=utils.today_ist),
) -> list[dict[str, Any]]:
    return data_processor.get_sr_history(_instrument_or_404(instrument), date)
